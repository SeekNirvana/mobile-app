...
class _VitalsScreenState extends ConsumerState<VitalsScreen> {
    ...
    switch (type) {
      case 'hr':
        ...
        _hrTimeout = Timer(_safetyTimeout, () {
          debugPrint(
            '[VitalsScreen] HR safety timeout - resetting measuring state',
          );
          if (mounted) {
      ...
      case 'spo2':
        ...
        _spo2Timeout = Timer(_safetyTimeout, () {
          debugPrint(
            '[VitalsScreen] SpO2 safety timeout - resetting measuring state',
          );
          if (mounted) {
      ...
      case 'temp':
        ...
        _tempTimeout = Timer(_safetyTimeout, () {
          debugPrint(
            '[VitalsScreen] Temperature safety timeout - resetting measuring state',
          );
          if (mounted) {
      ...
      case 'bp':
        ...
        _bpTimeout = Timer(_bpSafetyTimeout, () {
          debugPrint(
            '[VitalsScreen] BP safety timeout - resetting measuring state',
          );
          if (mounted) {
  ...
  void _startHrWaveform() {
    _hrWaveformTimer?.cancel();
    double t = 0;
    _hrWaveformTimer = Timer.periodic(const Duration(milliseconds: 50), (
      timer,
    ) {
      t += 0.15;
  ...
  void _startSpo2Waveform() {
    _spo2WaveformTimer?.cancel();
    double t = 0;
    _spo2WaveformTimer = Timer.periodic(const Duration(milliseconds: 50), (
      timer,
    ) {
      t += 0.12;
  ...
  void _startTempWaveform() {
    _tempWaveformTimer?.cancel();
    double t = 0;
    _tempWaveformTimer = Timer.periodic(const Duration(milliseconds: 80), (
      timer,
    ) {
      t += 0.08;
              ...
              SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: _VitalCardFullWidth(
                    ...
                    status: spo2 >= 95
                        ? 'Normal'
                        : (spo2 > 0 ? 'Low' : 'Not measured'),
                    isActive: spo2 > 0,
                    statusColor: spo2 >= 95
                        ? AppColors.success
                        : (spo2 > 0 ? AppColors.warning : null),
    ...
    if (isMeasuring) {
      ...
      RingPlugin.startBloodPressure().catchError((error) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Blood pressure scan could not start on this device.',
            ),
          ),
        );
      });
          ...
          children: [
            ...
            if (isMeasuring && waveformData.isNotEmpty)
              Container(
                ...
                child: _AnimatedWaveform(data: waveformData, color: iconColor),
              )
            else if (!isMeasuring &&
                historyData != null &&
